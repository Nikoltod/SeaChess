...
function checkForWinner() {
    ...
    for (var i = 0; i < paths.length; i++) {
        ...
        if (this.color(first[0], first[1], first[2]) == this.color(second[0], second[1], second[2]) &&
            ...
            $("#game-message-container").removeAttr("style");
            if (this.currentPlayer == "R") {
                messageEl.text("Red won the game!");
                console.log("whaat")
            } else {
                messageEl.text("Blue won the game!");
            }
...
function checkForStalemante() {
    ...
    if (result) {
        ...
        $("#game-message-container").removeAttr("style");
        $("#game-message").text("The game ended in a draw.");
    }
    return result;
}
...
function drawInnerCube(x, y, sides, frontLight, frontDark, rightLight, rightDark, topLight, topDark) {
    ...
    context.beginPath();
    context.moveTo(x - (sides / 2), y - (sides / 2));
    context.lineTo(x, y - (sides)); 
    context.lineTo(x + sides, y - sides); 
    context.lineTo(x + (sides / 2), y - (sides / 2)); 
    context.lineTo(x - (sides / 2), y - (sides / 2));
    context.closePath();
    context.fill();
...
function draw(board, selectedX, selectedY, selectedZ) {
    ...
    $(".tile").each(function() {
        ...
        $(this).removeClass("R");
        $(this).removeClass("B");
        if (board.color(x, y, z) != 'G') {
            $(this).addClass(board.color(x, y, z));
        }
    });
...
function doComputerMove(board) {
    ...
    $.ajax({
        url: "/Game/GetGameBoard/" + board.grid,
        success: function (serverState) {
            board.grid = serverState;
        }
    });
...
function pcVsPc(board) {
    ...
    if (!board.gameOver) {
        board.endTurn();
        setTimeout(function () { pcVsPc(board) }, 2000);
    }
}

function startNewGame() {
    $("#game-message-container").attr("style", "display:none");
    ...
    if (newBoard.player1 == "computer" && newBoard.player2 == "computer") {
        setTimeout(function () { pcVsPc(newBoard) }, 2000);
...
jQuery(function () {
    ...
    $(".tile").click(function () {
        ...
        if (!currentBoard.gameOver) {
            ...
            if (currentBoard.color(x, y, z) == 'G') {
                ...
                $.ajax({
                    url: "/Game/GetGameBoard/" + currentBoard.grid,
                    success: function (serverState) {
                        currentBoard.grid = serverState;
                        draw(currentBoard);
    ...
    $("li.option").click(function () {
        ...
        $.ajax({
            url: "/Game/EndGame/", success: function () {
                $.ajax({
                    url: "/Game/GetGameBoard/", success: function (data) {
                        currentBoard.grid = data;
                    }
                });
            }
        });
